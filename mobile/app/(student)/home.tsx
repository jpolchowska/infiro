...
export default function HomeScreen() {
  ...
  const currentPct = current ? Math.round((current.solvedTasks / current.totalTasks) * 100) : 0;
        ...
        >
          ...
          {stats && (
            <View className="flex-row mb-[22px]" style={{ gap: 10 }}>
              ...
              <StatTile value={stats.accuracy !== null ? `${stats.accuracy}%` : '—'} label="dobrych odpowiedzi" />
          ...
          >
            ...
            >
              <Text className="text-infiro-navy font-extrabold text-[13px]">
                {stats?.lastLevelingTest ? `${stats.lastLevelingTest.score}/${stats.lastLevelingTest.total}` : '—'}
              </Text>
            </View>
          ...
          <View style={{ gap: 12 }}>
            {(stats?.recentSections ?? []).map((section) => {
              ...
              const pct = Math.round((section.solvedTasks / section.totalTasks) * 100);
              return (
                <Pressable
                ...
                >
                  <View className="flex-1">
                    <Text className="text-infiro-navy font-extrabold text-base">{section.sectionTitle}</Text>
                    ...
                    >
                      <View className={accent.bg} style={{ height: 7, borderRadius: 100, width: `${pct}%` }} />
                    </View>
                  </View>
...
function StatTile({ value, label }: { value: string | number; label: string }) {
  return (
    ...
    >
      ...
      <Text style={{ color: '#8b93bd' }} className="font-semibold text-[12px] mt-1.5">
        {label}
      </Text>
    </View>
  );
