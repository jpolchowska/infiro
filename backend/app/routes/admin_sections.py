...
def create_task(subsection_id):
    ...
    for opt in options:
        if not isinstance(opt, dict) or not isinstance(opt.get("text"), str) or not opt["text"].strip():
            return jsonify({"error": "each option needs a non-empty 'text'"}), 400
        if not isinstance(opt.get("correct"), bool):
            return jsonify({"error": "each option needs a boolean 'correct'"}), 400
        if opt["correct"]:
            correct_count += 1

    if correct_count != 1:
        return jsonify({"error": "exactly one option must be correct"}), 400
